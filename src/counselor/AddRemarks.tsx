import axios from "axios";
import React, { useState } from "react";
import { ToastContainer, toast } from 'react-toastify';
import 'react-toastify/dist/ReactToastify.css';
 
import { useParams } from "react-router-dom";

const AddRemarks = () => {
  const { id } = useParams(); // 👈 Get `id` from the URL

  const [formData, setFormData] = useState({
    formNo: "",
    remarks: "",
  });

  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    const { name, value } = e.target;
    setFormData((prev) => ({
      ...prev,
      [name]: value,
    }));
  };

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();

    const token = localStorage.getItem("token");
    if (!token) {
      toast.error("Authentication token not found.");
      return;
    }

    try {
      const response = await axios.post(
        `http://localhost:3000/counselor/remarks/${id}`, // ✅ Fixed here
        formData,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        }
      );

      toast.success("Remarks added successfully!");
      console.log("Response:", response.data);

      setFormData({
        formNo: "",
        remarks: "",
      });

      setTimeout(() => {
        window.location.href = "/inquiryconformation";
      }, 1500);
    } catch (error: any) {
      console.error("Error adding remarks:", error);
      if (error.response?.data?.message) {
        toast.error(error.response.data.message);
      } else {
        toast.error("Something went wrong. Please try again.");
      }
    }
  };

  const handleReset = () => {
    setFormData({
      formNo: "",
      remarks: "",
    });
  };

  return (
    <div className="flex items-center justify-center p-4 bg-gray-50 min-h-screen">
      <ToastContainer position="top-right" autoClose={5000}  />
      <form
        onSubmit={handleSubmit}
        onReset={handleReset}
        className="w-full max-w-lg bg-white p-6 py-12 rounded-lg shadow-lg space-y-6"
      >
        <h2 className="text-2xl font-bold text-gray-700 mb-4 text-center">Add Remarks</h2>

        {/* <div>
          <label className="block text-sm font-medium text-gray-700">Form No.</label>
          <input
            name="formNo"
            value={formData.formNo}
            onChange={handleChange}
            placeholder="Enter Form No."
            className="mt-1 p-2 w-full border rounded-md focus:outline-none focus:ring-2 focus:ring-[#63589F] focus:border-transparent"
            required
          />
        </div> */}

        <div>
          <label className="block text-sm font-medium text-gray-700">Remarks</label>
          <textarea
            name="remarks"
            value={formData.remarks}
            onChange={handleChange}
            placeholder="Enter remarks"
            rows={4}
            className="mt-1 p-2 w-full border rounded-md focus:outline-none focus:ring-2 focus:ring-[#63589F] focus:border-transparent"
            required
          />
        </div>

        <div className="flex justify-between">
          <button
            type="submit"
            className="bg-[#63589F] text-white px-4 py-2 rounded-md hover:bg-[#D1AFE8] transition"
          >
            Submit
          </button>
          <button
            type="reset"
            className="bg-gray-400 text-white px-4 py-2 rounded-md hover:bg-gray-500 transition"
          >
            Reset
          </button>
        </div>
      </form>
    </div>
  );
};

export default AddRemarks;
